...
const methodTools = {
  ...
  Flashcards: {
    title: "Flashcards",
    type: "flashcards",
    fields: ["Front question/prompt", "Back definition/answer"],
  },
  "Practice questions": {
...
function applyFontSize(percent) {
  const size = Math.max(100, Math.min(112, Number(percent) || 100));
...
function bindFontSizeControl() {
  if (!fontSizeSlider) {
    applyFontSize(localStorage.getItem("discere:fontSize") || 100);
    return;
  }

  const key = getCurrentUser() ? getUserStorageKey("fontSize") : "discere:fontSize";
  const stored = localStorage.getItem(key) || localStorage.getItem("discere:fontSize") || "100";
  applyFontSize(stored);

  fontSizeSlider.addEventListener("input", () => {
    applyFontSize(fontSizeSlider.value);
    localStorage.setItem(key, fontSizeSlider.value);
  });
}
...
function requireLoggedIn() {
  ...
  if (currentUserLabel) {
    currentUserLabel.textContent = `Logged in: ${formatDisplayName(getCurrentUser())}`;
  }
...
function getSkillProfile() {
  const stored = JSON.parse(localStorage.getItem(getUserStorageKey("skillProfile")) || "{}");
  const baseline = stored.baseline || null;
  const events = Array.isArray(stored.events) ? stored.events.slice(-60) : [];
  const scoredEvents = events.filter((event) => Number.isFinite(Number(event.score)));
  const average = scoredEvents.length
    ? Math.round(scoredEvents.reduce((total, event) => total + Number(event.score), 0) / scoredEvents.length)
    : 0;
  const recent = scoredEvents.slice(-8);
  const recentAverage = recent.length
    ? Math.round(recent.reduce((total, event) => total + Number(event.score), 0) / recent.length)
    : average;
  const scoreLevel = recentAverage >= 85 ? "Advanced"
    : recentAverage >= 70 ? "Secure"
      : recentAverage >= 50 ? "Developing"
        : scoredEvents.length ? "Foundation" : "New";
  const level = scoredEvents.length ? scoreLevel : (baseline?.baselineLevel || "New");
  const nextAdjustment = level === "Advanced" ? "Gemini will create harder questions with fewer hints."
    : level === "Secure" ? "Gemini will mix standard and challenge questions."
      : level === "Developing" ? "Gemini will add clearer steps and model answers."
        : level === "Foundation" ? "Gemini will start with core knowledge and confidence checks."
          : "Complete a few quiz or practice questions to calibrate.";

  return {
    baseline,
    events,
    attempts: scoredEvents.length,
    average,
    recentAverage,
    level,
    nextAdjustment,
...
function saveSkillProfile(profile) {
  localStorage.setItem(getUserStorageKey("skillProfile"), JSON.stringify(profile));
}

function recordSkillEvent({ method, topic, score, rating }) {
  ...
  saveSkillProfile(nextProfile);
  renderSkillOverview();
}

function resetSkillProfile() {
  localStorage.removeItem(getUserStorageKey("skillProfile"));
  renderSkillOverview();
}

function buildLocalQuestionnaireOverview(answers, profileKey) {
  const answerLabels = answers.map((answer) => answer.answer);
  const needsShort = answerLabels.some((answer) => ["Less than 10 minutes", "10–20 minutes", "Shorter lessons"].includes(answer));
  const testingStress = answerLabels.some((answer) => ["I find them stressful", "I need extra time"].includes(answer));
  const distraction = answerLabels.some((answer) => ["Often", "Always", "Almost always", "Less distractions"].includes(answer));
  const baselineLevel = testingStress || needsShort ? "Foundation" : profileKey === "challenge" ? "Secure" : "Developing";
  return {
    source: "questionnaire",
    baselineLevel,
    confidence: baselineLevel === "Secure" ? 74 : baselineLevel === "Developing" ? 62 : 48,
    strength: profiles[profileKey]?.title || "Balanced learning habits",
    watch: distraction ? "Distractions or long sessions may lower focus." : "Keep checking answers so the overview can adapt.",
    focus: needsShort ? "Short focused tasks with quick checks." : "Active practice with clear feedback.",
    recommendedStart: testingStress ? "Start untimed, then add light time pressure later." : "Start with a few practice questions and mark them honestly.",
    questionnaireCount: answers.length,
  };
}

function initialiseSkillProfileFromQuestionnaire(answers, profileKey, overview = {}) {
  const baseline = {
    ...buildLocalQuestionnaireOverview(answers, profileKey),
    ...overview,
    source: overview?.source || "Gemini questionnaire",
    questionnaireCount: answers.length,
  };
  saveSkillProfile({
    baseline,
...
function getGeminiLearnerProfile() {
  const profile = getSkillProfile();
  return {
    skillLevel: profile.level,
    attempts: profile.attempts,
    averageScore: profile.average,
    recentAverage: profile.recentAverage,
    questionnaireBaseline: profile.baseline,
    instruction: profile.nextAdjustment,
  };
}

function renderSkillOverview() {
  const overview = document.querySelector("#skillOverview");
  if (!overview) return;
  const profile = getSkillProfile();
  const percent = profile.attempts ? `${profile.recentAverage}%` : "--";
  const rightCount = profile.events.filter((event) => event.rating === "good").length;
  const wrongCount = profile.events.filter((event) => event.rating === "again").length;
  const methods = profile.events.reduce((acc, event) => {
    if (!event.method) return acc;
    acc[event.method] = (acc[event.method] || 0) + 1;
    return acc;
  }, {});
  const topMethod = Object.entries(methods).sort((a, b) => b[1] - a[1])[0]?.[0] || "No method yet";
  const baseline = profile.baseline;
  overview.innerHTML = `
    <div class="overview-heading">
      <span>Revision overview</span>
      <strong>${escapeHtml(profile.level)} level</strong>
      <p>${escapeHtml(baseline?.focus || "Discere will adapt as you practise.")}</p>
    </div>
    <div class="skill-meter" aria-label="Recent accuracy">
      <span style="width: ${profile.attempts ? profile.recentAverage : 8}%"></span>
    </div>
    <div class="overview-stats">
      <article><span>Checked</span><strong>${profile.attempts}</strong></article>
      <article><span>Recent</span><strong>${percent}</strong></article>
      <article><span>Right</span><strong>${rightCount}</strong></article>
      <article><span>Retry</span><strong>${wrongCount}</strong></article>
    </div>
    <div class="overview-notes">
      <p><b>Questionnaire:</b> ${escapeHtml(baseline?.strength || "No baseline yet.")}</p>
      <p><b>Watch:</b> ${escapeHtml(baseline?.watch || "Answer a few questions so Discere can spot patterns.")}</p>
      <p><b>Next:</b> ${escapeHtml(profile.nextAdjustment)}</p>
      <p><b>Most practised:</b> ${escapeHtml(topMethod)}</p>
    </div>
    <button class="side-mini-button" type="button" id="resetSkillProfile">Reset overview</button>
  `;
  overview.querySelector("#resetSkillProfile").addEventListener("click", resetSkillProfile);
}
...
function updateSideToggleButtons(collapsed) {
  document.querySelectorAll("#sideToggle, [data-side-toggle]").forEach((button) => {
    const isMainToggle = button.id === "sideToggle";
    button.textContent = isMainToggle ? (collapsed ? ">" : "<") : (collapsed ? "Show side menu" : "Hide side menu");
...
function bindSidePanelToggle() {
  ...
  button.addEventListener("click", () => {
    window.setTimeout(() => updateSideToggleButtons(document.body.classList.contains("side-collapsed")), 0);
  });
  document.addEventListener("click", (event) => {
    if (!event.target.closest("[data-side-toggle]")) return;
    const collapsed = !document.body.classList.contains("side-collapsed");
    document.body.classList.toggle("side-collapsed", collapsed);
    updateSideToggleButtons(collapsed);
    localStorage.setItem(getUserStorageKey("sideCollapsed"), collapsed ? "yes" : "no");
  });
  updateSideToggleButtons(document.body.classList.contains("side-collapsed"));
}
...
function renderRevisionSpace(studySpace) {
  ...
  document.querySelector("#resultName").textContent = `${formatDisplayName(name)}'s ${profile.title.toLowerCase()} space`;
  ...
  if (!skillProfile.baseline && answers.length) {
    saveSkillProfile({
      baseline: buildLocalQuestionnaireOverview(answers, profileKey),
      events: skillProfile.events,
      updatedAt: skillProfile.updatedAt || new Date().toISOString(),
    });
  }
...
function renderQuizGenerator(toolName, tool) {
  return `
    <form class="tool-form" id="toolForm">
      <div class="form-intro quiz-builder-intro">
        <div>
        ...
        <button class="ghost-button quiz-side-toggle" type="button" data-side-toggle aria-expanded="${document.body.classList.contains("side-collapsed") ? "false" : "true"}">
          ${document.body.classList.contains("side-collapsed") ? "Show side menu" : "Hide side menu"}
        </button>
      </div>
      <div class="ai-generator-panel">
        <div class="field-grid">
...
function renderInteractiveMindMap(map, options = {}) {
  ...
  const radiusX = 34;
  const radiusY = 30;
  const branchPoints = branches.map((branch, index) => {
    const angle = branches.length === 1 ? -Math.PI / 2 : (-Math.PI / 2) + (index * 2 * Math.PI / branches.length);
    return {
      branch,
      index,
  ...
  const notePoints = branchPoints.flatMap(({ branch, index, x, y, angle }) => {
    const details = Array.isArray(branch.details) ? branch.details.slice(0, 4) : [];
    return details.map((detail, detailIndex) => {
      const offset = (detailIndex - (details.length - 1) / 2) * 0.34;
      const noteAngle = angle + offset;
      return {
        detail,
        branchIndex: index,
        detailIndex,
        fromX: x,
        fromY: y,
        x: centre.x + Math.cos(noteAngle) * 46,
        y: centre.y + Math.sin(noteAngle) * 42,
      };
    });
  });
    ...
    <div class="interactive-mindmap" data-mindmap data-editable="${editable ? "true" : "false"}" data-item-id="${escapeHtml(itemId)}" data-map="${encodedMap}">
      <div class="mindmap-canvas">
        <svg class="mindmap-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
          ${branchPoints.map((point) => `<line class="mindmap-line branch-line" x1="${centre.x}" y1="${centre.y}" x2="${point.x}" y2="${point.y}"></line>`).join("")}
          ${notePoints.map((point) => `<line class="mindmap-line note-line" x1="${point.fromX}" y1="${point.fromY}" x2="${point.x}" y2="${point.y}"></line>`).join("")}
        </svg>
        <div class="mindmap-node mindmap-root" style="left:${centre.x}%; top:${centre.y}%;">${escapeHtml(map?.centralIdea || "Mind map")}</div>
        ${branchPoints.map(({ branch, index, x, y }) => `
          <button class="mindmap-node mindmap-branch" style="left:${x}%; top:${y}%;" type="button" data-map-branch="${index}">
            <strong>${escapeHtml(branch.title || `Branch ${index + 1}`)}</strong>
            <span>${Array.isArray(branch.details) ? branch.details.length : 0} notes</span>
          </button>
        `).join("")}
        ${notePoints.map((point) => `
          <button class="mindmap-note-node" style="left:${point.x}%; top:${point.y}%;" type="button" data-map-branch="${point.branchIndex}">
            ${escapeHtml(point.detail)}
          </button>
        `).join("")}
      </div>
      ${editable ? `
...
function bindQuizGenerator(toolName) {
  ...
  button.addEventListener("click", async () => {
    ...
    const learnerProfile = getGeminiLearnerProfile();

    try {
      const result = await postJson("/api/quiz", { topic, level, specifics, count, learnerProfile });
      ...
      const generatedItems = questions.map((item) => ({
        id: makeId(),
        fields: [String(item.question || "Generated question").trim(), String(item.answer || "No answer supplied.").trim()],
        practised: 0,
        due: Date.now(),
        generatedBy: "Gemini",
        quizTitle: result.title || topic || "Generated quiz",
        learnerLevel: learnerProfile.skillLevel,
      })).filter((item) => item.fields[0] && item.fields[1]);
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      const result = await postJson("/api/revision-helper", {
        ...
        specifics,
        fields: tool.fields,
        learnerProfile: getGeminiLearnerProfile(),
      });
...
function bindQuestionPractice(toolName, tool, saved) {
  ...
  document.querySelectorAll("[data-rate]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const manualScore = button.dataset.rate === "good" ? Math.max(autoScore, 80) : Math.min(autoScore || 35, 45);
      recordSkillEvent({
        method: toolName,
        topic: getPracticeQuestion(tool, saved[index]),
        score: manualScore,
        rating: button.dataset.rate,
      });
      const nextItems = getSaved(toolName).map((item) => item.id === saved[index].id
        ? { ...item, practised: (item.practised || 0) + 1, autoRating, autoScore: manualScore, lastRating: button.dataset.rate }
        : item);
      saveItems(toolName, nextItems);
...
function initQuestionnairePage() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    let questionnaireOverview = buildLocalQuestionnaireOverview(answers, profileKey);
    let usedGemini = false;

    statusMessage.textContent = "Asking Gemini...";

    try {
      const geminiResult = await geminiCategorise(answers);
      profileKey = geminiResult.profile;
      questionnaireOverview = {
        ...questionnaireOverview,
        ...(geminiResult.overview || {}),
        source: "Gemini questionnaire",
    ...
    initialiseSkillProfileFromQuestionnaire(answers, profileKey, questionnaireOverview);
